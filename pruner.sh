...
for OLD_JOB in $OLD_JOBS; do
  ...
  echo "$INSTANCES" | format_for_deletion zone | xargs -L 1 gcloud compute instances delete --quiet

  DISKS=$(gcloud compute disks list --format json --filter "labels: $OLD_JOB" --format json)
  echo "$DISKS" | format_for_deletion zone | xargs -L 1 gcloud compute disks delete --quiet
  ...
  echo "$BACKEND_SVC" | format_for_deletion region | xargs gcloud compute backend-services delete --quiet

  INST_GROUPS=$(gcloud compute instance-groups list --filter "name ~ $OLD_JOB" --format json)
  echo "$INST_GROUPS" |  format_for_deletion zone | xargs -L 1 gcloud compute instance-groups unmanaged delete --quiet
  ...
  echo "$HEALTH_CHECKS" |  jq -r '.[].name' | xargs gcloud compute health-checks delete --quiet

  TARGET_POOLS=$(gcloud compute target-pools list --filter "name ~ $OLD_JOB" --format json)
  echo "$TARGET_POOLS" | format_for_deletion region  | xargs -L 1 gcloud compute target-pools delete --quiet
  ...
  echo "$SVC_ACCTS" | xargs -L 1 gcloud iam service-accounts delete --quiet

  echo "finding and deleting storage buckets..."
  BUCKETS=$(gcloud storage buckets list --filter "name ~ $OLD_JOB" --format json)
  OBJECTS=$(echo "$BUCKETS" | jq -r '.[].storage_url' | xargs gcloud storage objects list --format json)
  echo "$OBJECTS" | jq -r '.[].storage_url' | xargs gcloud storage rm --quiet
  echo "$BUCKETS" | jq -r '.[].storage_url' | xargs gcloud storage buckets delete --quiet
  ...
  if [ -n "$DNS_ZONE" ] ; then
    ...
    echo "$DNS_RECORDS" | jq -r '.[] | select(.type=="A") | .name' | xargs -L 1 gcloud dns record-sets delete --zone "$DNS_ZONE" --type A 
