...
def wait_for_rosa_cluster_to_be_hibernated(cluster:utils.OcCluster, worker_count:int):
    time.sleep(15)
    ec2_map = utils.get_instances_for_region(cluster.region, 'stopped')
...
def wait_for_ipi_cluster_to_be_hibernated(cluster:utils.OcCluster, worker_count:int):
    ...
    print(f"Waiting for {len(InstanceIds)} worker nodes to stop, please wait...", flush=True)
    while len(InstanceIds) < worker_count:
        print(f'\t{len(InstanceIds)}/{worker_count} nodes are stopped, will check again in 5s...', flush=True)
        time.sleep(5)
        ec2_map = utils.get_instances_for_region(cluster.region, 'stopped')
...
def main():
    ...
    if len(target_cluster) == 1:
        ...
        ec2_map = utils.get_instances_for_region(target_cluster.region, 'running')
        ...
        if target_cluster.hcp == "false":
            if target_cluster.type == 'ocp':
                ...
                if target_cluster.status == "ready":
                    ...
                    print(
                        f'Cluster {target_cluster.name} is not in ready state, please wait for it to be ready and try again', flush=True)
        else:
            hibernate_hypershift_cluster(target_cluster, ec2_map)
